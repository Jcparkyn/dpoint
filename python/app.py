...
class CanvasWrapper:
    def __init__(self, eng):
        ...
        self.cube = visuals.Box(0.014, stylus_len, 0.014, edge_color="black",
                         parent=self.view_top.scene)
        pen_tip = visuals.XYZAxis(parent=self.cube)
        pen_tip.transform = transforms.MatrixTransform(vispy.util.transforms.scale([0.02, 0.02, 0.02]) @ vispy.util.transforms.translate([0, 0, stylus_len*0.5]))
        self.cube.transform = transforms.MatrixTransform(vispy.util.transforms.translate([0, 0, 0]))

        trail_data = np.zeros((TRAIL_POINTS, 3), dtype=np.float32)
        self.trail_line = visuals.Line(pos=trail_data, color="red", width=2, parent=self.view_top.scene)
    ...
    def update_data(self, new_data_dict):
        ...
        orientation_quat = quaternion.Quaternion(orientation[0], orientation[1], orientation[3], orientation[2])
        or_fix = vispy.util.transforms.rotate(90, [1, 0, 0])
        pos = new_data_dict["position"]
        pos_converted = np.array([-pos[0], pos[1], -pos[2]])
        self.cube.transform.matrix = vispy.util.transforms.translate([0, 0, stylus_len*0.5]) @ orientation_quat.get_matrix() @ or_fix @ vispy.util.transforms.translate(pos_converted)
...
class ImuDataSource(QtCore.QObject):
    ...
    def __init__(self, eng: matlab.engine.MatlabEngine, imuf, tracker_queue: mp.Queue, num_iterations=50000, parent=None):
    ...
    def run_data_creation(self):
        ...
        for _ in range(self._num_iters):
            ...
            try:
                ...
                self._eng.update_tracker(self._imuf, camera_position, camera_orientation, nargout=0)
            except queue.Empty:
                pass
            ...
            if len(imu_queue) > 3:
                ...
                task = self._eng.step(self._imuf, accel, gyro, dt, nargout=2, background=True)
                # task.done
...
def run_tracker_with_queue(queue: mp.Queue):
    run_tracker(lambda orientation, position: queue.put((orientation, position), block=False))
...
if __name__ == "__main__":
    ...
    data_source.finished.connect(data_thread.quit, QtCore.Qt.ConnectionType.DirectConnection)
    # if the window is closed, tell the data source to stop
    win.closing.connect(data_source.stop_data, QtCore.Qt.ConnectionType.DirectConnection)
