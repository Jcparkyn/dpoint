...
markersOnPen = {
    ...
    6: getCornersPS(np.array([0, 0.1175, 0.011], dtype=np.float32), deg2rad(270)),
...
baseMarkerCornersWS = np.array([
    ...
    [baseMarkerLength/2, baseMarkerLength/2, 0],
    [baseMarkerLength/2, -baseMarkerLength/2, 0],
    [-baseMarkerLength/2, -baseMarkerLength/2, 0]], dtype=np.float32)
...
def run_tracker(on_estimate: Optional[Callable[[np.ndarray, np.ndarray], None]]):
  ...
  while(True):
    ...
    if len(validMarkers) >= 1:
      ...
      rvecRelative, tvecRelative = relativeTransform(rvec, tvec, baseRvec, baseTvec)
      ...
      cv2.putText(frame, f"Tip: [{array_to_str(tvecRelative)}]", (10, 120), cv2.FONT_HERSHEY_DUPLEX, 1, (0, 255, 0))
    ...
    cv2.putText(frame, f"FPS: {avg_fps:.1f}", (10, 30), cv2.FONT_HERSHEY_DUPLEX, 1, (0, 255, 0))
    cv2.putText(frame, f"Processing: {(frameEndTime - processingStartTime)*1000:.1f}ms", (10, 60), cv2.FONT_HERSHEY_DUPLEX, 1, (0, 255, 0))
