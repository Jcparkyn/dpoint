...
async def monitor_ble_async(queue: mp.Queue):
    ...
    def queue_notification_handler(characteristic: BleakGATTCharacteristic, data: bytearray):
        accel, gyro, t = unpack_imu_data_packet(data)
        queue.put((accel, gyro, t))

    disconnected_event = asyncio.Event()
    async with BleakClient(device, disconnected_callback=disconnected_event.set) as client:
